...
bool Options::SetOptionsInputVector() {
	input_vector = true;
	return true;
}
...
void WordTable::PrintAll()
{
	...
	std::cout << "total cols: " << cols << " total words: " << total_words << endl;
}
...
void SequenceDB::Read(const char* file, const Options& options)
{
	...
	while (not feof(fin) || one.size) { /* do not break when the last sequence is not handled */
		...
		// 使用 fgets 从文件读取一行至 buffer。
		if ((res = fgets(buffer, MAX_LINE_SIZE, fin)) == NULL && one.size == 0)
			break;

		// 处理质量分数行，此时读取质量分数行的标志行，主要针对FASTQ格式
		if (buffer[0] == '+') {
			// 有+ 和 '\n' 两个字符
			int len = strlen(buffer);
			int len2 = len;
...
void SequenceDB::updateParent(vector<int>& parent){
	...
	for (int i = 0; i < N; i++) {
		...
		int root_id = sequences[rep_seqs[sequences[i]->cluster_id]]->index;
		parent[id] = root_id;
	}
...
//		int id = seq->index;
//		int root_id = sequences[rep_seqs[seq->cluster_id]]->index;
//		parent[id] = root_id;
//		cout << id << " " << root_id << endl;
//	}
...
void SequenceDB::printSequences() {
	for (size_t i = 0;i < sequences.size();i++) {
		std::cout << "Identifier: " << sequences[i]->identifier << " Size:" << sequences[i]->size << std::endl;
	}
}
...
void SequenceDB::WriteClusters(const char* db, const char* newdb, const Options& options)
{
	...
	if (fin == NULL || fout == NULL)
		bomb_error("file opening failed");
	// for (i = 0;i < n;i++) {
	// 	std::cout << sequences[rep_seqs[i]]->identifier << " " << sequences[rep_seqs[i]]->size << " " << rep_seqs[i] << "  " << sequences[rep_seqs[i]]->index << std::endl;
	// }
	for (i = 0; i < n; i++)
		sorting[i] = ((uint64_t)sequences[rep_seqs[i]]->index << 32) | rep_seqs[i];
	std::sort(sorting.begin(), sorting.end());
	for (i = 0; i < n; i++) {
		Sequence* seq = sequences[sorting[i] & 0xffffffff];
		fseek(fin, seq->des_begin, SEEK_SET);
...
void SequenceDB::WriteClustersvector(const std::vector<Input_Sequence*>& input, const char* newdb, const Options& options) {
	FILE* fout = fopen(newdb, "w+");
	int i, j, n = rep_seqs.size();
	int count, rest;
	vector<uint64_t> sorting(n);
	...
	for (i = 0; i < n; i++) {
		Sequence* seq = sequences[sorting[i] & 0xffffffff];
		int index = seq->des_begin;
		const char* identifier = input[index]->identifier;
		if (identifier) {
			size_t len = strlen(identifier);

			// Optionally add a newline if needed
			fwrite(identifier, 1, len, fout);

			// Check if the identifier lacks a newline, and append one
			if (identifier[len - 1] != '\n') {
				fputc('\n', fout);
			}
		}
		const char* data = input[index]->data;
		if (data) {
...
void SequenceDB::WriteExtra1D(const Options& options)
{
	...
	// std::cout << std::endl << ">>>>>>>" << options.sort_output << std::endl;
